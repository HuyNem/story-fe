...
function CommentComponent(props) {
    ...
    //api comment
    //mutation
    const mutationComment = useMutationHooks(
        (data) => {
    ...
    const queryComment = useQuery({ queryKey: ['comment'], queryFn: fetchGetComment, enabled: !!state.id });
    const { isPending, data: comments } = queryComment;
    const dataWithKeys = comments?.map(item => ({ ...item, key: item._id }));

    return (
        <Wrapper>
            <h3>NHẬN XÉT CỦA BẠN VỀ TRUYỆN NÀY</h3>
            <hr />
            <TextArea showCount maxLength={200} className='text-area' name='comment' onChange={handleOnChangeComment}></TextArea>
            <button onClick={send}>Gửi <IoIosSend /></button>
            <List
                ...
                renderItem={(item, index) => (
                    <List.Item>
                        <List.Item.Meta
                            title={item.nameUser}
                            description={<div style={{ textAlign: 'justify', paddingRight: 20 }}>{item.content}</div>}
                        />
